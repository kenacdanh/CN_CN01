...
class CentralServer(Base):
    ...
    def peer_register(self, msgdata):
        ...
        if peer_name in self.peerlist:
            self.client_send(
                (peer_host, peer_port), msgtype="REGISTER_ERROR", msgdata={}
            )
            ...
            self.client_send(
                (peer_host, peer_port), msgtype="REGISTER_SUCCESS", msgdata={}
            )
    ...
    def peer_login(self, msgdata):
        ...
        if peer_name in self.peerlist:
            ...
            if str(peer_password) == peer_pass_retrieved:
                ...
                self.client_send(
                    (peer_host, peer_port), msgtype="LOGIN_SUCCESS", msgdata={}
                )
                ...
                # noti
                print(
                    peer_name, " has been added to central server's online user list!"
                )

            else:
                self.client_send(
                    (peer_host, peer_port), msgtype="LOGIN_ERROR", msgdata={}
                )
    ...
    def peer_list(self, msgdata):
        ...
        self.client_send(
            (peer_host, peer_port), msgtype="LIST_ONLINE_USER", msgdata=data
        )
    ...
    def peer_logout(self, msgdata):
        ...
        if peer_name in self.onlinelist:
            ...
            # noti
            print(
                peer_name, " has been removed from central server's online user list!"
            )
    ...
    def peer_broadcast_chat(self, msgdata):
        ...
        for x in self.onlinelist:
            self.client_send(self.onlinelist[x], msgtype="CHAT_PUBLIC", msgdata=msgdata)
