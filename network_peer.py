...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(
            ...
            bg="#1E1E1E",
            fg="#00FF00",  # Green text and background
            height="2",
            font=("Verdana", 13),
        ).pack(fill="x")
        ...
        tk.Label(
            self,
            text="Set Port (1 -> 9999)",
            bg="#1E1E1E",
            fg="#00FF00",
            font=("Courier New", 11),
        ).pack()  # Green text and background
        # Set port entry
        ...
        )  # Green text, background, and insertion cursor
        ...
        tk.Button(
            ...
            border=0,
            font=("Courier New", 11),
            command=lambda: self.enter_app(
                controller=controller, port=self.port_entry.get(), page=RegisterPage
            ),
        ).pack()  # Green text and background
        tk.Label(self, text="", bg="#1E1E1E").pack()  # Set background color for space
        ...
        tk.Button(
            ...
            font=("Courier New", 11),
            command=lambda: self.enter_app(
                controller=controller, port=self.port_entry.get(), page=LoginPage
            ),
        ).pack()  # Green text and background
...
class RegisterPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Username
        tk.Label(
            self, text="Username", bg="#1E1E1E", fg="#00FF00", font=("Courier New", 11)
        ).place(x=670, y=175)
        ...
        # Password
        tk.Label(
            self, text="Password", bg="#1E1E1E", fg="#00FF00", font=("Courier New", 11)
        ).place(x=670, y=250)
...
class PrivateChatPage(tk.Frame):
    ...
    def sendFile(self, friend_name):
        ...
        file_path = tkinter.filedialog.askopenfilename(
            initialdir="/", title="Select a File", filetypes=filetypes
        )
        file_name = os.path.basename(file_path)
        msg_box = tkinter.messagebox.askquestion(
            "File Explorer",
            "Are you sure to send {} to {}?".format(file_name, friend_name),
            icon="question",
        )
        if msg_box == "yes":
            ...
            tkinter.messagebox.showinfo(
                "File Transfer",
                "{} has been sent to {}!".format(file_name, friend_name),
            )
    ...
    def open_chatroom(self):
        ...
        line = tk.Label(chatroom, width=450, bg="#ABB2B9")

        line.place(relwidth=1, rely=0.07, relheight=0.012)

        app.chatroom_textCons = tk.Text(
            chatroom,
            width=20,
            height=2,
            bg="#17202A",
        ...
        app.chatroom_textCons.place(relheight=0.745, relwidth=1, rely=0.08)

        labelBottom = tk.Label(chatroom, bg="#ABB2B9", height=80)

        labelBottom.place(relwidth=1, rely=0.825)

        self.entryMsg = tk.Entry(
            labelBottom, bg="#2C3E50", fg="#EAECEE", font="Helvetica 13"
        )
        ...
        self.buttonMsg = tk.Button(
            ...
            width=20,
            bg="#ABB2B9",
            command=lambda: self.sendPublicChatButton(self.entryMsg.get()),
        )
...
class ChatPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        mycanvas_2.bind(
            "<Configure>",
            lambda e: mycanvas_2.configure(scrollregion=mycanvas_2.bbox("all")),
        )

        friend_frame = tk.Frame(mycanvas_2)
        mycanvas_2.create_window((0, 0), window=friend_frame)

        reload_button_2 = tk.Button(
            friend_frame,
            image=reload_icon,
            border=0,
            background="#f1e8fc",
        ...
        if len(network_peer.onlinelist) > 0:
            ...
            for online_user in network_peer.onlinelist:
                ...
                self.user_name_frame[i].config(
                    command=lambda online_user=online_user: self.make_chat_request(
                        online_user
                    )
                )
                i = i + 1
...
class NetworkPeer(Base):
    def __init__(
        ...
        self.server_info = (self.serverhost, 40000)
    ...
    def register_error(self, msgdata):
        ...
        display_noti("Register Noti", "Register Error. Username existed or null!")
    ...
    def chat_request(self, msgdata):
        ...
        msg_box = tk.messagebox.askquestion(
            "Chat Request",
            "Do you want to accept {} - {}:{}?".format(peername, host, port),
            icon="question",
        )
        if msg_box == "yes":
            ...
            display_noti(
                "Chat Request Accepted", "Update to get in touch with new friend!"
            )
    ...
    def chat_accept(self, msgdata):
        ...
        display_noti(
            "Chat Request Result",
            "CHAT ACCEPTED: {} --- {}:{}. Update to get in touch with new friend!".format(
                peername, host, port
            ),
        )
        self.friendlist[peername] = (host, port)
    ...
    def sendPublicMessage(self, message):
        ...
        try:
            data = {"name": self.name, "message": message}
    ...
    def send_chat_message(self, friend, message):
        ...
        try:
            ...
            data = {"friend_name": self.name, "message": message}
    ...
    def recv_file_content(self):
        ...
        while True:
            ...
            file_name = (
                str(random.randint(1, 100000000)) + "_" + recv_file_info["filename"]
            )
            ...
            display_noti(
                ...
                "You receive a file with name " + file_name + " from " + friend_name,
            )
